	...
	<div class="row">
		...
		<div class="breadcrumbs d-flex flex-row align-items-center">
			<ul>
				...
				<li class="active"><a href="#"><i class="fa fa-angle-right" aria-hidden="true"></i>Thống kế nông sản</a></li>
			</ul>
		</div>
	</div>
	<div class="row">
		<div class="tab-content" id="myTabContent">
			<div class="tab-pane fade show active" id="product" role="tabpanel" aria-labelledby="product-tab">
				<div class="form-input">
					<form class="form-inline" id="frmProduct" name="frmProduct" method="POST">
						@csrf
				...
				<div class="chart">
					<canvas id="chartProduct" width="1000" height="500"></canvas>
				</div>
			</div>
...
<script>
	...
	//thống kê nông sản
	$('#frmProduct').on('submit', function(event){
		event.preventDefault();

		$.ajax({
			url: '{{ route('trader.statistical.product') }}',
			type: 'POST',
			data: {
			...
			success: function(res){
				...
				var myChart = new Chart(ctx, {
					type: 'bar',
					data: {
						labels: res.list_name,
						datasets: [
						{
							label: 'Sản lượng nông sản',
					...
					option: {
						title: {
							display: true,
							position: "top",
							title: "Thống kê sản lượng nông sản",
							fontSize: 18,
							fontColor: "#111",
						},
						legend: {
			...
			error: function(xhr, ajaxOptions, thrownError){
				toastr['error']('Gặp sự cố khi thống kê nông sản');
			}
		})
	})
</script>
