...
<script>
	...
	$('#frmTransaction').on('submit', function(event){
		...
		$.ajax({
			url: '{{ route('trader.statistical.transaction') }}',
			type: 'POST',
			data: {
			...
			success: function(res){
				...
				var myChart = new Chart(ctx, {
					type: 'line',
					data: {
						labels: res.import.list_time,
						datasets: [
						{
							label: 'Giao dịch nhập hàng',
							backgroundColor: 'rgba(100, 200, 300, 0.35)',
							borderColor:" #8e5ea2",
							borderWidth: 1,
							hoverBackgroundColor: 'rgba(200, 200, 200, 1)',
							hoverBorderColor: 'rgba(200, 200, 200, 1)',
							data: res.import.list_quantity,		
						},
						{
							label: 'Giao dịch xuất hàng',
							backgroundColor: 'rgba(10, 20, 30, 0.35)',
							borderColor: "#3e95cd",
							borderWidth: 1,
							hoverBackgroundColor: 'rgba(20, 20, 20, 1)',
							hoverBorderColor: 'rgba(20, 20, 20, 1)',
							data: res.export.list_quantity,	
						}
						],						
					},
					options: {
